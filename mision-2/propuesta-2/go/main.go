// Propuesta #2: vector ordenado + contador único de corridas contiguas.
package main
...
type gradeModeApp struct {
	...
	globalVector *grades.GradeVector
	filesOK      int
...
func newGradeModeApp() *gradeModeApp {
	return &gradeModeApp{
		...
		globalVector: grades.NewGradeVector(),
...
func (app *gradeModeApp) run() {
	fmt.Println("=== Calificación más común (Propuesta #2: vector ordenado + contador único) ===")
...
func (app *gradeModeApp) runAutoDir(dir string) error {
	fmt.Printf("=== Calificación más común (Propuesta #2: vector ordenado + contador único) ===\n")
...
func (app *gradeModeApp) processFile(fileName string) {
	...
	fileVector := grades.NewGradeVector()
	result, err := app.reader.ReadInto(fileName, fileVector)
	if err != nil {
		fmt.Printf("Error: %v\n", err)
		return
	}
	...
	fileVector.Sort()
	mode := fileVector.MostCommonGrade()
	elapsed := time.Since(start)

	if result.LinesSkipped > 0 {
		fmt.Printf("Aviso: se ignoraron %d línea(s) inválida(s) en %q.\n", result.LinesSkipped, fileName)
	}
	fmt.Println(app.analyzer.DescribeFile(fileName, fileVector.Total(), mode, elapsed))

	app.globalVector.Merge(fileVector)
...
func (app *gradeModeApp) printGlobalResult() {
	fmt.Println()
	if app.filesOK == 0 || app.globalVector.IsEmpty() {
		fmt.Println("No se procesó ningún archivo válido. Fin del programa.")
		return
	}

	modeStart := time.Now()
	app.globalVector.Sort()
	globalMode := app.globalVector.MostCommonGrade()
	modeElapsed := time.Since(modeStart)

	fmt.Println(app.analyzer.DescribeGlobal(app.globalVector.Total(), globalMode, modeElapsed, app.totalElapsed))
}
