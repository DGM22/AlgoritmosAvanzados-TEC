...
func newGradeModeApp() *gradeModeApp {
	return &gradeModeApp{
		...
		globalTable:  grades.NewHashTable(),
...
func (app *gradeModeApp) run() {
	fmt.Println("Calificación más común (Propuesta #3: tabla hash con encadenamiento externo) ")
...
func (app *gradeModeApp) runAutoDir(dir string) error {
	fmt.Printf("=== Calificación más común (Propuesta #3: tabla hash con encadenamiento externo) ===\n")
